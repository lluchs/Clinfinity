...
public func Timer() {
	...
	while(node = HashIterNext(iter))
		...
		node[1] -> UpdateCount( GetAmount(node[0]) );
}
...
public func MaterialCheck(id idType)
{
	...
	while(node = HashIterNext(iter))
		...
		node[1] -> CompareCount( GetComponent(node[0], 0, 0, idType), GetAmount(node[0]) );
}

public func MaterialCheckCancel() // sollte vom Bausatz in MenuQueryCancel() aufgerufen werden
{
	var iter = HashIter(hIcons), node;
	while(node = HashIterNext(iter))
		node[1] -> ClearComparison();
}
